...
`cobalt-trace-slo-dock` is centered on observability work in Scala. The technical target is: Package a Scala local lab for slo analysis with bounded scenario files, conflict explanations, and documented operating limits.
...
The score is deterministic. The current threshold is 170. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 6.
